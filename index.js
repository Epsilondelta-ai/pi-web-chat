var M={attachFile:["M16.5 6v11.5a4 4 0 0 1-8 0V5a2.5 2.5 0 0 1 5 0v10.5a1 1 0 0 1-2 0V6H10v9.5","a2.5 2.5 0 0 0 5 0V5a4 4 0 0 0-8 0v12.5a5.5 5.5 0 0 0 11 0V6h-1.5Z"].join(""),stop:"M6 6h12v12H6V6Z",send:"M2 21 23 12 2 3v7l15 2-15 2v7Z",terminal:"M3 4h18v16H3V4Zm2 2v12h14V6H5Zm2 3 1.1-1.1L12.2 12l-4.1 4.1L7 15l3-3-3-3Zm6 6h5v1.5h-5V15Z",file:"M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6Zm0 2.5L17.5 8H14V4.5ZM8 13h8v1.5H8V13Zm0 3h8v1.5H8V16Z"},P={attachFile:$("attach_file",M.attachFile),stop:$("stop",M.stop),send:$("send",M.send)};function $(i,p){return`<svg class="material-icon" data-material-icon="${i}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="${p}"></path></svg>`}function U(){return"pi-web-chat-mounted"}function W(){let i=document.createElement("main");return i.className="main pi-web-chat-surface",i.dataset.main="session",i.innerHTML='<div class="term"><div class="term-inner" role="log" aria-live="polite" aria-relevant="additions text"></div></div><button type="button" class="scroll-bottom-btn" data-action="scroll-bottom" aria-label="scroll to bottom" title="scroll to bottom" hidden>↓</button>',i}function v(){let i=document.createElement("section");return i.className="prompt-region pi-web-chat-composer",i.innerHTML=`
  ...
  `}function I(){document.getElementById("pi-web-chat-style")?.remove();let i=document.createElement("style");return i.id="pi-web-chat-style",i.textContent=S(),document.head.append(i),i}function m(i){return i.command||i.cmd||(i.name?`/${i.name}`:"")}function a(i){if(!i)throw Error("pi-web-chat DOM template is invalid");return i}function Hi(i,p,r){return{threadId:i,runId:p,state:{},messages:r,tools:[],context:[]}}function Mi(i){let p=[...i.messages].reverse().find((r)=>r.role==="user");return{text:p?.text||"",attachments:p?.attachments||[],sessionId:i.threadId}}function $i(i,p,r){return i.map((b)=>xi(b,p,r))}function xi(i,p,r){if(i.type==="text.delta")return{type:"TEXT_MESSAGE_CONTENT",threadId:p,runId:r,delta:i.delta||""};if(i.type==="thinking.delta")return{type:"THINKING_MESSAGE_CONTENT",threadId:p,runId:r,delta:i.delta||""};if(i.type==="tool.start")return{type:"TOOL_CALL_START",threadId:p,runId:r,toolCallId:i.toolCallId,payload:o(i)};if(i.type==="tool.delta")return{type:"TOOL_CALL_ARGS",threadId:p,runId:r,toolCallId:i.toolCallId,delta:i.delta||""};if(i.type==="tool.end")return{type:"TOOL_CALL_END",threadId:p,runId:r,toolCallId:i.toolCallId,payload:o(i)};return{type:i.type.toUpperCase().replaceAll(".","_"),threadId:p,runId:r,payload:i}}function o(i){return{name:i.toolName||"tool",args:i.args||{},result:i.result||"",isError:i.isError===!0}}var D="pi-web-chat.sessions.v1";var bi="plugin.pi-web-sidebar.selectedSession",hi="plugin.pi-web-sidebar.event",ci="plugin.pi-web-sidebar.activeSessionId",G="plugin.pi-web-sidebar.activeWorkspaceId";var t=20,u=200,ui=8,Ni=1e6;var ki={"book-open":'<path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/>',"file-plus":'<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M12 18v-6"/><path d="M9 15h6"/>',pencil:'<path d="M21.2 6.8 17.2 2.8a2 2 0 0 0-2.8 0L3 14.2V21h6.8L21.2 9.6a2 2 0 0 0 0-2.8z"/><path d="m14 5 5 5"/>',terminal:'<path d="m4 17 6-6-6-6"/><path d="M12 19h8"/>',cpu:'<rect x="6" y="6" width="12" height="12" rx="2"/><path d="M9 1v3"/><path d="M15 1v3"/><path d="M9 20v3"/><path d="M15 20v3"/><path d="M20 9h3"/><path d="M20 15h3"/><path d="M1 9h3"/><path d="M1 15h3"/>',globe:'<circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 0 20"/><path d="M12 2a15.3 15.3 0 0 0 0 20"/>',search:'<circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>',braces:'<path d="M8 3H7a2 2 0 0 0-2 2v4a2 2 0 0 1-2 2 2 2 0 0 1 2 2v4a2 2 0 0 0 2 2h1"/><path d="M16 21h1a2 2 0 0 0 2-2v-4a2 2 0 0 1 2-2 2 2 0 0 1-2-2V7a2 2 0 0 0-2-2h-1"/>',"file-text":'<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/>',"circle-question-mark":'<circle cx="12" cy="12" r="10"/><path d="M9.1 9a3 3 0 1 1 5.8 1c-.5 1-1.4 1.5-2.1 2-.6.4-.8.8-.8 2"/><path d="M12 17h.01"/>',users:'<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>',plug:'<path d="M12 22v-5"/><path d="M9 8V2"/><path d="M15 8V2"/><path d="M18 8v5a6 6 0 0 1-12 0V8Z"/>',"git-branch":'<line x1="6" x2="6" y1="3" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/>',"circle-check":'<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>',hammer:'<path d="m15 12-8.5 8.5a2.1 2.1 0 0 1-3-3L12 9"/><path d="m17.6 5.4 1.8-1.8"/><path d="m14 7 3 3"/><path d="M5 11 2 8l6-6 3 3"/>',package:'<path d="m7.5 4.3 9 5.2"/><path d="M21 8a2 2 0 0 0-1-1.7l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.7l7 4a2 2 0 0 0 2 0l7-4a2 2 0 0 0 1-1.7Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>'},zi={read:"book-open",write:"file-plus",edit:"pencil",ctx_execute:"cpu",ctx_batch_execute:"cpu",ctx_execute_file:"cpu",ctx_search:"cpu",ctx_index:"cpu",ctx_fetch_and_index:"cpu",web_search:"globe",fetch_content:"search",get_search_content:"search",code_search:"braces",document_parse:"file-text",ask_user_question:"circle-question-mark",teams:"users",subagent:"users",mcp:"plug",git:"git-branch"};class li{#i=[];add(i){if(i)this.#i.push(i);return i}listen(i,p,r){i.addEventListener(p,r),this.add({remove:()=>i.removeEventListener(p,r)})}dispose(){for(let i of this.#i.splice(0).reverse())if(typeof i==="function")i();else if("unsubscribe"in i)i.unsubscribe();else i.remove()}}function Fi(i={}){let p=i.app;if(p?.piWebChat?.dispose(),typeof i.mount?.chat!=="function"||typeof i.mount?.composer!=="function")throw Error("pi-web-chat requires modern pi-web mount.chat and mount.composer APIs");return Pi(i,p)}function Pi(i,p){let r=new li,b=r.add(I()),h=W(),c=v(),l=i.mount?.chat(h,{replace:!0}),f=i.mount?.composer(c,{replace:!0});if(l)r.add(l);if(f)r.add(f);let g=q(i);z(i,g||void 0);let w=Lp(g?.sessionId||""),L={backendChatToken:0};_(h,y(w).messages),j(i,h,w,L,g?.sessionId||w.activeSessionId),Ii(r,i,h,w,L),Ui(r,i,c,h,w,L);let T=p?r.add(n(p)):void 0;p?.classList.add(U());let O=()=>{if(L.runEventsAbort?.abort(),L.sessionEventsAbort?.abort(),r.dispose(),T?.remove(),b.remove(),p?.classList.remove(U()),p?.piWebChat===H)delete p.piWebChat},H={dispose:O};if(p)p.piWebChat=H;return O}function Ui(i,p,r,b,h,c){let l=r.querySelector(".prompt-textarea"),f=r.querySelector(".send-btn"),g=r.querySelector(".attach-btn"),w=r.querySelector("[data-file-input]"),L=r.querySelector(".attach-chips");if(!l||!f)return;let T=[],O=()=>{let C=l.value;f.setAttribute("aria-disabled",C.trim()?"false":"true")},H=()=>{if(!L)return;d(L,T.map((C)=>{return C.name||"attachment"}))},X=async(C)=>{C?.preventDefault(),C?.stopImmediatePropagation();let E=l.value.trim();if(O(),!E)return;let Q=[...T];f.disabled=!0;try{if(Yi(E,Q),await Vi(p,b,h,c,E,Q),T=[],H(),l.value.trim()===E)l.value="";if(w)w.value=""}catch(_i){_(b,[Zi(_i)])}finally{f.disabled=!1,O()}};if(i.listen(l,"input",O),i.listen(l,"keydown",(C)=>{let E=C;if(E.key==="Enter"&&(E.metaKey||E.ctrlKey))X(E)}),i.listen(f,"click",(C)=>{X(C)}),g&&w)i.listen(g,"click",()=>w.click()),i.listen(w,"change",()=>{Bi(w,(C)=>{T=C,H()})})}function Zi(i){return{id:F(),role:"system",text:`prompt failed: ${Ei(i)}`,createdAt:Date.now()}}async function Bi(i,p){let r=Array.from(i.files||[]).slice(0,ui),b=[];for(let h of r){if(h.size>Ni)continue;b.push({name:h.name,size:h.size,content:await h.text(),mimeType:h.type||void 0})}p(b)}function up(i){return{input$:i.behaviorSubject("chat.input",""),submitted$:i.subject("chat.input.submitted"),activeSessionId$:i.behaviorSubject("session.activeId",null),sidebarSelectedSession$:i.behaviorSubject(bi,Di()),toastRequested$:i.subject("toast.requested")}}function Np(i){let p=[],r=new Set,b=/(^|[\s`])@([^\s@`]+)/g,h=b.exec(String(i||""));while(h!==null){let c=h[2]||"";if((h[1]||"")!=="`"&&c&&!r.has(c))r.add(c),p.push(c);h=b.exec(String(i||""))}return p}function kp(i=[],p=[]){let r=[],b=new Set;for(let h of[...i,...p]){let c=m(h);if(!c||b.has(c))continue;b.add(c),r.push(h)}return r}function x(i){return i.app?.piWebSidebar?.getSnapshot?.().activeWorkspaceId||i.app?.dataset.activeWorkspaceId||""}function Di(){return q({})}function q(i){let p=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),r=p?.activeSessionId||"",b=p?.activeWorkspaceId||"";if(r)return{sessionId:r,workspaceId:b||void 0};let h=V(ci),c=V(G);return h?{sessionId:h,workspaceId:c||void 0}:null}function z(i,p){if(p?.workspaceId&&i.app)i.app.dataset.activeWorkspaceId=p.workspaceId,e(G,p.workspaceId);if(p?.sessionId)i.app?.setAttribute("data-active-session-id",p.sessionId),e(ci,p.sessionId)}function N(i,p,r={}){let b=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),h={type:p,detail:r,snapshot:b};(i.app?.piWebSidebar?.channels?.events$||globalThis.piWebSidebar?.channels?.events$)?.next(h),globalThis.piWeb?.subject(hi).next(h)}function fi(i,p,r){let b=i.app?.dataset.activeWorkspaceId||x(i),h={reason:r,sessionId:p,workspaceId:b};globalThis.piWeb?.behaviorSubject("session.activeId",p).next(p),globalThis.piWeb?.subject("session.changed").next({sessionId:p,workspaceId:b,reason:r}),N(i,"active.start",h),N(i,"session.created",h)}function e(i,p){try{localStorage.setItem(i,p)}catch{}}async function J(i,p,r={}){if(!i.backend)return{};let b=x(i),h=await i.backend(p,{workspaceId:b,data:r});return R(h)?h:{}}async function Vi(i,p,r,b,h,c){b.backendChatToken+=1,b.runEventsAbort?.abort(),b.runEventsAbort=new AbortController,b.sessionEventsAbort?.abort();let l={id:F(),role:"user",text:h,attachments:Oi(c),createdAt:Date.now()},f=y(r);f.messages.push(l),f.updatedAt=Date.now(),A(r),_(p,f.messages);let g=await Ki(i,h,c,r.activeSessionId);if(typeof g.activeSessionId==="string"&&g.activeSessionId){let L=r.activeSessionId,T=K(r,g.activeSessionId);if(L!==T.id&&!T.messages.some((O)=>O.id===l.id))T.messages.push(l),T.updatedAt=Date.now();z(i,{sessionId:g.activeSessionId,workspaceId:i.app?.dataset.activeWorkspaceId||x(i)||void 0}),N(i,"chat-session",{reason:"startPrompt",sessionId:g.activeSessionId}),fi(i,g.activeSessionId,"startPrompt")}if(typeof g.runId!=="string"||!g.runId){let L=await Qi(i,h,c,r.activeSessionId),T=Y(i,r,L,"submitPrompt");_(p,T);return}let w=Wi(r);try{await Gi(i,r,g.runId,w,()=>_(p,y(r).messages),b.runEventsAbort.signal)}finally{b.runEventsAbort=void 0}j(i,p,r,b,r.activeSessionId)}async function Gi(i,p,r,b,h,c){let l=await wi(i,"streamEventsSse",{runId:r,cursor:0},c);if(!l)throw Error("SSE streaming backend did not return a stream");b.streaming=!0,h();try{await gi(l,(f)=>{vi(b,[f]),b.streaming=f.type!=="run.end",y(p).updatedAt=Date.now(),A(p),h()})}finally{b.streaming=!1,A(p),h()}}async function wi(i,p,r={},b){if(!i.backendStream)throw Error("SSE streaming backend is unavailable");let h=x(i),c=await i.backendStream(p,{workspaceId:h,data:r},{signal:b});return qi(c)}function qi(i){if(typeof ReadableStream<"u"&&i instanceof ReadableStream)return i;if(typeof Response<"u"&&i instanceof Response)return i.body;if(typeof i==="string")return s(i);if(!R(i))return null;let p=i.body;if(typeof ReadableStream<"u"&&p instanceof ReadableStream)return p;let r=i.sse;if(typeof r==="string")return s(r);return null}function s(i){let p=new TextEncoder().encode(i);return new ReadableStream({start(r){r.enqueue(p),r.close()}})}async function gi(i,p){let r=i.getReader(),b=new TextDecoder,h="";while(!0){let c=await r.read();if(c.done){h+=b.decode(),ji(h,p);return}h+=b.decode(c.value,{stream:!0});let l=Ji(h);h=l.remainder;for(let f of l.frames)Li(f,p)}}function Ji(i){let r=i.replaceAll(`\r
`,`
`).replaceAll("\r",`
`).split(`

`),b=r.pop()||"";return{frames:r,remainder:b}}function ji(i,p){let r=i.trim();if(r)Li(r,p)}function Li(i,p){let r=i.split(`
`).filter((b)=>b.startsWith("data:")).map((b)=>b.slice(5).trimStart()).join(`
`);if(!r)return;try{let b=JSON.parse(r);if(ni(b))p(b)}catch{}}function Yi(i,p){globalThis.piWeb?.subject("chat.input.submitted").next({text:i,attachments:Oi(p)||[]})}async function Ki(i,p,r,b){try{return await J(i,"startPrompt",{text:p,attachments:r,sessionId:b})}catch(h){if(Xi(h))return{};throw h}}function Xi(i){return/unknown method: (startPrompt|streamEventsSse)|unsupported method: (startPrompt|streamEventsSse)|(startPrompt|streamEventsSse) unsupported/i.test(Ei(i))}async function Qi(i,p,r,b=""){return await J(i,"submitPrompt",{text:p,attachments:r,sessionId:b})}function Wi(i){let p=y(i),r=[...p.messages].reverse().find((h)=>h.role==="assistant"&&h.streaming);if(r)return r;let b={id:F(),role:"assistant",text:"",createdAt:Date.now(),streaming:!0};return p.messages.push(b),b}function vi(i,p){for(let r of p)if(r.type==="text.delta"&&typeof r.delta==="string")i.text+=r.delta;else if(r.type==="thinking.delta"&&typeof r.delta==="string")i.thinking=`${i.thinking||""}${r.delta}`;else if(r.type==="tool.start")Z(i,r,"running");else if(r.type==="tool.delta"&&typeof r.delta==="string"){let b=Z(i,r,"running");b.text=r.delta}else if(r.type==="tool.end"){let b=Z(i,r,r.isError?"err":"ok");if(typeof r.result==="string"&&r.result)b.text=r.result}else if(r.type==="error"&&typeof r.message==="string")i.text+=`${i.text?`
`:""}${r.message}`}function Z(i,p,r){let b=p.toolCallId||p.toolName||"tool";i.toolCalls||=[];let h=i.toolCalls.find((c)=>c.id===b);if(!h)h={id:b,name:p.toolName||"tool",args:p.args,text:"",status:r},i.toolCalls.push(h);if(h.status=r,p.args)h.args=p.args;return h}function ni(i){return R(i)&&typeof i.type==="string"}async function j(i,p,r,b,h=""){if(!i.backendStream){await ii(i,p,r,b,h);return}let c=++b.backendChatToken;b.sessionEventsAbort?.abort();let l=new AbortController;b.sessionEventsAbort=l;try{let f=await wi(i,"sessionEventsSse",Ti(i,h),l.signal);if(!f)throw Error("session SSE backend did not return a stream");await gi(f,(g)=>{if(c!==b.backendChatToken||g.type!=="chat.state")return;let w=Y(i,r,di(g),"chatState");if(w.length)_(p,w)})}catch(f){if(!l.signal.aborted)await ii(i,p,r,b,h)}finally{if(b.sessionEventsAbort===l)b.sessionEventsAbort=void 0}}async function ii(i,p,r,b,h=""){let c=++b.backendChatToken;try{let l=await J(i,"chatState",Ti(i,h));if(c!==b.backendChatToken)return;let f=Y(i,r,l,"chatState");if(f.length)_(p,f)}catch{}}function Ti(i,p){let r=p?{sessionId:p}:{},b=Si(i);if(b)r.workspacePath=b;return r}function di(i){return{activeSessionId:i.activeSessionId,messages:i.messages,isStreaming:i.isStreaming}}function Si(i){let p=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),r=i.app?.dataset.activeWorkspaceId||p?.activeWorkspaceId||"";return p?.workspaces?.find((b)=>b.id===r)?.path||""}function Y(i,p,r,b){let h=ai(r.messages);if(typeof r.activeSessionId==="string"&&r.activeSessionId){let l=p.activeSessionId;if(K(p,r.activeSessionId),z(i,{sessionId:r.activeSessionId,workspaceId:i.app?.dataset.activeWorkspaceId||x(i)||void 0}),b!=="chatState"||l!==r.activeSessionId)N(i,"chat-session",{reason:b,sessionId:r.activeSessionId});if(b!=="chatState")fi(i,r.activeSessionId,b)}let c=y(p);if(h.length){if(c.messages=gp(c.messages,h).slice(-u),c.title==="New chat"){let l=c.messages.find((f)=>f.role==="user");if(l)c.title=l.text.slice(0,48)||c.title}c.updatedAt=Date.now()}return A(p),c.messages}function Ii(i,p,r,b,h){let c=(w)=>{if(!w?.sessionId)return;if(w.sessionId!==b.activeSessionId)h.runEventsAbort?.abort();z(p,w),K(b,w.sessionId),_(r,y(b).messages),j(p,r,b,h,w.sessionId)},l=(w)=>{let L=mi(p,w);if(L)c(L)},f=p.app?.piWebSidebar?.channels?.selectedSession$||globalThis.piWebSidebar?.channels?.selectedSession$,g=p.app?.piWebSidebar?.channels?.events$||globalThis.piWebSidebar?.channels?.events$;if(f)i.add(f.subscribe(c));if(g)i.add(g.subscribe(l));if(p.app)i.listen(p.app,"pi-web-sidebar:session-created",(w)=>{let L=w.detail||{},T=typeof L.sessionId==="string"?L.sessionId:"",O=typeof L.workspaceId==="string"?L.workspaceId:"";if(T)c({sessionId:T,workspaceId:O||void 0})});if(globalThis.piWeb)i.add(globalThis.piWeb.behaviorSubject(bi,q(p)).subscribe(c)),i.add(globalThis.piWeb.subject(hi).subscribe(l)),i.add(globalThis.piWeb.behaviorSubject("session.activeId",null).subscribe((w)=>{if(!w)return;c({sessionId:w,workspaceId:p.app?.dataset.activeWorkspaceId||V(G)||void 0})}))}function mi(i,p){if(!oi(p.type))return null;let r=p.detail||{},b=typeof r.sessionId==="string"?r.sessionId:p.snapshot?.activeSessionId||"",h=typeof r.workspaceId==="string"?r.workspaceId:p.snapshot?.activeWorkspaceId||i.app?.dataset.activeWorkspaceId||"";if(!b)return null;return{sessionId:b,workspaceId:h||void 0}}function oi(i){return i==="session.selected"||i==="session.created"||i==="new-session"||i==="active.start"}function K(i,p){let r=i.sessions.find((b)=>b.id===p);if(!r)r=k(p),i.sessions.unshift(r);return i.activeSessionId=r.id,A(i),r}function _(i,p){(i.querySelector(".term-inner")||i).replaceChildren(...p.map(ti))}function ti(i){let p=document.createElement("article");p.className="transcript-item",p.dataset.messageId=i.id;let r=document.createElement("div");r.className="msg",r.dataset.kind=B(i.role);let b=document.createElement("span");b.className=`prefix ${B(i.role)}`,b.textContent=wp(i.role);let h=document.createElement("pre");if(h.className=`body ${B(i.role)}`,h.textContent=i.text,r.append(b,h),p.append(r),i.thinking)p.append(ei(i.thinking,Boolean(i.streaming)));for(let c of i.toolCalls||[])p.append(si(c));if(i.streaming)p.dataset.streaming="true";return p}function ei(i,p){let r=document.createElement("details");r.className="msg-detail think thinking-block",r.open=p;let b=document.createElement("summary");b.className="label",b.textContent="THINKING";let h=document.createElement("pre");return h.className="body",h.textContent=i,r.append(b,h),r}function si(i){let p=document.createElement("div");p.className="tool-card",p.dataset.tool=i.name||"tool",p.dataset.status=i.status;let r=i.status!=="running";p.dataset.collapsed=r?"true":"false";let b=document.createElement("button");b.type="button",b.className="tc-head",b.title=r?"Show tool output":"Hide tool output",b.setAttribute("aria-expanded",r?"false":"true"),b.setAttribute("aria-label",`${r?"Show":"Hide"} ${i.name||"tool"} output`),b.append(pp(i),rp(i),bp(i),hp(i));let h=document.createElement("pre");return h.className="tc-body",h.hidden=r,h.textContent=i.text||JSON.stringify(i.args||{},null,2),b.addEventListener("click",()=>ip(p,b,h)),p.append(b,h),p}function ip(i,p,r){let b=r.hidden===!1;r.hidden=b,i.dataset.collapsed=b?"true":"false",p.setAttribute("aria-expanded",b?"false":"true"),p.setAttribute("aria-label",`${b?"Show":"Hide"} ${i.dataset.tool||"tool"} output`),p.title=b?"Show tool output":"Hide tool output";let h=p.querySelector(".tc-toggle-label");if(h)h.textContent=b?"show":"hide"}function pp(i){let p=document.createElement("span");p.className="tc-glyph";let r=cp(i),b=r?ki[r]:"";if(!r||!b)return p.textContent="●",p;return p.innerHTML=`<svg class="tc-icon" data-tool-icon="${r}" aria-hidden="true" viewBox="0 0 24 24">${b}</svg>`,p}function rp(i){let p=document.createElement("span");return p.className="tc-name",p.textContent=i.name||"tool",p}function bp(i){let p=document.createElement("span");return p.className="tc-args",p.textContent=Ci(i),p}function hp(i){let p=document.createElement("span");if(p.className="tc-meta",i.status==="running"){let h=document.createElement("span");h.className="spinner",h.textContent="⠇";let c=document.createElement("span");return c.className="running",c.textContent="running",p.append(h,c,pi(i.status)),p}let r=document.createElement("span");r.className=i.status==="err"?"err":"ok",r.textContent=i.status==="err"?"✗":"✓";let b=document.createElement("span");return b.textContent=i.status==="err"?" · failed":" · done",p.append(r,b,pi()),p}function pi(i="ok"){let p=document.createElement("span");p.className="tc-toggle";let r=document.createElement("span");r.className="tc-toggle-label",r.textContent=i==="running"?"hide":"show";let b=document.createElement("span");return b.className="tc-caret",b.textContent="▸",p.append(r,b),p}function Ci(i){if(!i.args)return"";return JSON.stringify(i.args)}function cp(i){let p=lp(i.name);if(["bash","shell","sh","zsh","terminal"].includes(p))return fp(Ci(i).toLowerCase())||"terminal";return zi[p]}function lp(i){return i.trim().replace(/^functions\./,"").replace(/^multi_tool_use\./,"")}function fp(i){if(/\bgit\b|\bgh\b/.test(i))return"git-branch";if(/\b(go test|bun test|npm test|pnpm test|yarn test|vitest|pytest|cargo test)\b/.test(i))return"circle-check";if(/\b(bun|npm|pnpm|yarn)\s+(run\s+)?build\b|\b(go build|cargo build)\b/.test(i))return"hammer";if(/\b(bun|npm|pnpm|yarn)\s+(add|install|i)\b|\bpip\s+install\b|\bgo\s+get\b/.test(i))return"package";return}function wp(i){if(i==="assistant")return"pi >";if(i==="user")return"you >";if(i==="system")return"sys >";return"tool >"}function B(i){if(i==="assistant")return"pi";if(i==="system")return"sys";return i}function gp(i,p){let r=new Map;for(let b of i)r.set(b.id,b);for(let b of p)r.set(b.id,{...r.get(b.id),...b});return[...r.values()].sort((b,h)=>b.createdAt-h.createdAt)}function y(i){let p=i.sessions.find((r)=>r.id===i.activeSessionId);if(!p)p=k(),i.sessions.unshift(p),i.activeSessionId=p.id,A(i);return p}function k(i=F()){let p=Date.now();return{id:i,title:"New chat",createdAt:p,updatedAt:p,messages:[]}}function V(i){try{return localStorage.getItem(i)||""}catch{return""}}function Lp(i=""){try{let r=JSON.parse(localStorage.getItem(D)||"null");if(r&&typeof r.activeSessionId==="string"&&Array.isArray(r.sessions)){let b=r.sessions.filter(Tp).map(Cp);if(i&&!b.some((h)=>h.id===i))b.unshift(k(i));return{activeSessionId:i||r.activeSessionId,sessions:b}}}catch{}let p=k(i||void 0);return{activeSessionId:p.id,sessions:[p]}}function A(i){ri(i);try{localStorage.setItem(D,JSON.stringify(i))}catch{for(let p of i.sessions)p.messages=p.messages.slice(-Math.floor(u/2));ri(i);try{localStorage.setItem(D,JSON.stringify(i))}catch{}}}function Oi(i){if(!i?.length)return;return i.map(({content:p,...r})=>r)}function ri(i){i.sessions.sort((b,h)=>h.updatedAt-b.updatedAt);let p=i.sessions.find((b)=>b.id===i.activeSessionId),r=i.sessions.filter((b)=>b.id!==i.activeSessionId).slice(0,Math.max(0,t-1));i.sessions=p?[p,...r]:i.sessions.slice(0,t);for(let b of i.sessions)if(b.messages.length>u)b.messages.splice(0,b.messages.length-u)}function Tp(i){return R(i)&&typeof i.id==="string"&&Array.isArray(i.messages)}function Cp(i){return{...i,messages:ai(i.messages)}}function ai(i){return Array.isArray(i)?i.filter(_p).map(Op):[]}function Op(i){if(!Array.isArray(i.toolCalls)){let{toolCalls:p,...r}=i;return r}return{...i,toolCalls:i.toolCalls.filter(ap)}}function ap(i){if(!R(i)||typeof i.id!=="string"||typeof i.name!=="string")return!1;return typeof i.text==="string"&&Ep(i.status)}function Ep(i){return i==="running"||i==="ok"||i==="err"}function _p(i){if(!R(i)||typeof i.id!=="string"||typeof i.text!=="string")return!1;return typeof i.createdAt==="number"&&yp(i.role)}function yp(i){return i==="user"||i==="assistant"||i==="tool"||i==="system"}function R(i){return typeof i==="object"&&i!==null}function F(){return`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`}function Ei(i){return i instanceof Error?i.message:String(i)}export{Ai as renderMessages,Mi as promptFromAgUiLikeRunInput,S as pluginStyleText,U as pluginClass,kp as mergeCommands,x as getActiveWorkspaceId,Np as extractRefs,Fi as default,v as createComposerSurface,W as createChatSurface,yi as createChatDom,up as createChannels,Hi as createAgUiLikeRunInput,m as commandName,$i as chatEventsToAgUiLikeEvents,J as backendCall};
