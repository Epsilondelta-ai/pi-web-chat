...
${li(l)}`}w.updatedAt=Date.now(),E(b),R(p,w.messages,f)}async function Ap(i,p){let b=Bi(p);if(!b.length)return[];let h=await V(i,"resolveContext",{text:p,refs:b});return Array.isArray(h.attachments)?h.attachments.filter(A):[]}function Fp(i,p,b,h,f){let w=Up(p,Op),r=f||w.truncated,c=w.text.endsWith(`
`)||!w.text?"":`
`;return`$ ${i}
${w.text}${c}[exit ${b} · ${h}ms${r?" · truncated":""}]`}function Up(i,p){let b=new TextEncoder,h=b.encode(i);if(h.byteLength<=p)return{text:i,truncated:!1};let f=new TextDecoder().decode(h.slice(0,p));while(f&&b.encode(f).byteLength>p)f=f.slice(0,-1);return{text:f,truncated:!0}}async function Zp(i,p,b,h,f,w){let r=p.querySelector(".slash-pop"),c=p.querySelector(".slash-list"),l=Eb(h);if(l===null||!c){r?.setAttribute("hidden","");return}let L=f;if(!L.length){let O=await V(i,"commands",{});L=Array.isArray(O.commands)?O.commands.filter(A):[],w(L)}if(b.value!==h)return;let T=L.filter((O)=>{return k(O).slice(1).toLowerCase().includes(l)});jp(c,T,b,r),r?.toggleAttribute("hidden",T.length===0)}async function Vp(i,p,b,h){let f=p.querySelector(".prompt-file-ref-pop"),w=p.querySelector(".prompt-file-ref-list"),r=Hb(h);if(r===null||!w){f?.setAttribute("hidden","");return}try{let c=await V(i,"searchFiles",{query:r,limit:cp}),l=Array.isArray(c.files)?c.files.filter(A):[];if(b.value!==h)return;Dp(w,b,l,f),f?.toggleAttribute("hidden",l.length===0)}catch{f?.setAttribute("hidden","")}}function jp(i,p,b,h){i.replaceChildren(...p.map((f)=>{let w=document.createElement("button"),r=k(f);return w.type="button",w.className="slash-item",w.dataset.slash=r,w.innerHTML='<span class="sl-name"></span><span class="sl-desc"></span>',w.querySelector(".sl-name").textContent=r,w.querySelector(".sl-desc").textContent=f.description||"",w.addEventListener("click",()=>{b.value=f.template||`${r} `,h?.setAttribute("hidden",""),b.dispatchEvent(new(b.ownerDocument.defaultView?.Event||Event)("input",{bubbles:!0})),b.focus()}),w}))}function Dp(i,p,b,h){i.replaceChildren(...b.map((f)=>{let w=f.path||f.name||"",r=document.createElement("button");return r.type="button",r.className="prompt-file-ref-item",r.dataset.path=w,r.disabled=!w,r.innerHTML='<span class="pfr-path"></span><span class="pfr-kind"></span>',r.querySelector(".pfr-path").textContent=w,r.querySelector(".pfr-kind").textContent=typeof f.size==="number"?`${f.size} bytes`:"file",r.addEventListener("click",()=>{Gp(p,w),h?.setAttribute("hidden","")}),r}))}function Gp(i,p){if(!p)return;let{value:b,selectionStart:h}=i,f=b.slice(0,h),w=/(?:^|\s)@([^\s@`]*)$/.exec(f),r=w?h-(w[1]||"").length-1:h,c=`@${p} `;i.value=`${b.slice(0,r)}${c}${b.slice(h)}`;let l=r+c.length;i.setSelectionRange(l,l),i.dispatchEvent(new(i.ownerDocument.defaultView?.Event||Event)("input",{bubbles:!0})),i.focus()}function Yb(i){return{input$:i.behaviorSubject("chat.input",""),submitted$:i.subject("chat.input.submitted"),activeSessionId$:i.behaviorSubject("session.activeId",null),sidebarSelectedSession$:i.behaviorSubject(Di,Jp()),toastRequested$:i.subject("toast.requested")}}function Bi(i){let p=[],b=new Set,h=/(^|[\s`])@([^\s@`]+)/g,f=h.exec(String(i||""));while(f!==null){let w=f[2]||"";if((f[1]||"")!=="`"&&w&&!b.has(w))b.add(w),p.push(w);f=h.exec(String(i||""))}return p}function Xb(i=[],p=[]){let b=[],h=new Set;for(let f of[...i,...p]){let w=k(f);if(!w||h.has(w))continue;h.add(w),b.push(f)}return b}function S(i){return i.app?.piWebSidebar?.getSnapshot?.().activeWorkspaceId||i.app?.dataset.activeWorkspaceId||""}function Jp(){return K({})}function K(i){let p=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),b=p?.activeSessionId||"",h=p?.activeWorkspaceId||"";if(b)return{sessionId:b,workspaceId:h||void 0};let f=d(pi),w=d(a);return f?{sessionId:f,workspaceId:w||void 0}:null}function Z(i,p){if(p?.workspaceId&&i.app)i.app.dataset.activeWorkspaceId=p.workspaceId,Ri(a,p.workspaceId);if(p?.sessionId)i.app?.setAttribute("data-active-session-id",p.sessionId),Ri(pi,p.sessionId)}function qp(i){i.app?.removeAttribute("data-active-session-id"),Bp(pi),globalThis.piWeb?.behaviorSubject("session.activeId",null).next(null)}function Q(i,p,b={}){let h=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),f={type:p,detail:b,snapshot:h};(i.app?.piWebSidebar?.channels?.events$||globalThis.piWebSidebar?.channels?.events$)?.next(f),globalThis.piWeb?.subject(Gi).next(f)}function bi(i,p,b){let h=i.app?.dataset.activeWorkspaceId||S(i),f={reason:b,sessionId:p,workspaceId:h};globalThis.piWeb?.behaviorSubject("session.activeId",p).next(p),globalThis.piWeb?.subject("session.changed").next({sessionId:p,workspaceId:h,reason:b}),Q(i,"active.start",f),Q(i,"session.created",f)}function Ri(i,p){try{localStorage.setItem(i,p)}catch{}}function Bp(i){try{localStorage.removeItem(i)}catch{}}async function V(i,p,b={},h=S(i)){if(!i.backend)return{};let f=await i.backend(p,{workspaceId:h,data:b});return A(f)?f:{}}function zi(i){let p=Array.isArray(i.warnings)?i.warnings.filter((b)=>typeof b==="string"&&b.trim().length>0):[];for(let b of p)globalThis.piWeb?.subject("toast.requested").next({level:"warning",message:b})}function Mb(i,p){return i.trim().startsWith("!")?[]:[...p]}async function Yp(i,p,b,h,f,w){let r=ui(i,b),c=N(i);h.backendChatToken+=1,h.runEventsAbort?.abort();let l=new AbortController;h.runEventsAbort=l,h.sessionEventsAbort?.abort();let L={id:D(),role:"user",text:f,attachments:si(w),createdAt:Date.now()},T=U(b,r);T.messages.push(L),T.updatedAt=Date.now(),E(b),R(p,T.messages,r);let O=await Wp(i,f,w,r,c.path,c.id);if(zi(O),typeof O.activeSessionId==="string"&&O.activeSessionId){let H=r,z=b.activeSessionId===H;if(r=O.activeSessionId,z)G(b,r),Z(i,{sessionId:O.activeSessionId,workspaceId:c.id||void 0}),Q(i,"chat-session",{reason:"startPrompt",sessionId:O.activeSessionId}),bi(i,O.activeSessionId,"startPrompt");else U(b,r);$b(b,H,r,L.id)}if(typeof O.runId!=="string"||!O.runId){let H=await vp(i,f,w,r,c.path,c.id);zi(H);let z=ap(i,b,H,"submitPrompt",r);if(b.activeSessionId===r)R(p,z,r);if(h.runEventsAbort===l)h.runEventsAbort=void 0;return}let $=U(b,r),F=kp($);try{await Xp(i,b,$,O.runId,c.path,c.id,F,()=>{if(b.activeSessionId===r)R(p,$.messages,r)},l.signal)}finally{if(h.runEventsAbort===l)h.runEventsAbort=void 0}if(b.activeSessionId===r)n(i,p,b,h,r,c.path,c.id)}async function Xp(i,p,b,h,f,w,r,c,l){let L=await Xi(i,"streamEventsSse",{runId:h,cursor:0,workspacePath:f},l,w);if(!L)throw Error("SSE streaming backend did not return a stream");let T=Yi(c);r.streaming=!0,T.flush();try{await Mi(L,(O)=>{up(r,[O]),r.streaming=O.type!=="run.end",b.updatedAt=Date.now(),E(p),T.request()})}finally{r.streaming=!1,E(p),T.flush()}}function Yi(i){let p=0,b,h=()=>{if(b)clearTimeout(b),b=void 0},f=()=>{h(),p=Date.now(),i()};return{request:()=>{let r=gp-(Date.now()-p);if(r<=0){f();return}b||=setTimeout(f,r)},flush:f,cancel:h}}async function Xi(i,p,b={},h,f=S(i)){if(!i.backendStream)throw Error("SSE streaming backend is unavailable");let w=await i.backendStream(p,{workspaceId:f,data:b},{signal:h});return Mp(w)}function Mp(i){if(typeof ReadableStream<"u"&&i instanceof ReadableStream)return i;if(typeof Response<"u"&&i instanceof Response)return i.body;if(typeof i==="string")return Ni(i);if(!A(i))return null;let p=i.body;if(typeof ReadableStream<"u"&&p instanceof ReadableStream)return p;let b=i.sse;if(typeof b==="string")return Ni(b);return null}function Ni(i){let p=new TextEncoder().encode(i);return new ReadableStream({start(b){b.enqueue(p),b.close()}})}async function Mi(i,p){let b=i.getReader(),h=new TextDecoder,f="";while(!0){let w=await b.read();if(w.done){f+=h.decode(),Kp(f,p);return}f+=h.decode(w.value,{stream:!0});let r=Qp(f);f=r.remainder;for(let c of r.frames)Qi(c,p)}}function Qp(i){let b=i.replaceAll(`\r
`,`
`).replaceAll("\r",`
`).split(`

`),h=b.pop()||"";return{frames:b,remainder:h}}function Kp(i,p){let b=i.trim();if(b)Qi(b,p)}function Qi(i,p){let b=i.split(`
`).filter((h)=>h.startsWith("data:")).map((h)=>h.slice(5).trimStart()).join(`
`);if(!b)return;try{let h=JSON.parse(b);if(Ip(h))p(h)}catch{}}function Ai(i,p){globalThis.piWeb?.subject("chat.input.submitted").next({text:i,attachments:si(p)||[]})}async function Wp(i,p,b,h,f=N(i).path,w=N(i).id){try{return await V(i,"startPrompt",Ki(p,b,h,f),w)}catch(r){if(Pp(r))return{};throw r}}function Pp(i){return/unknown method: (startPrompt|streamEventsSse)|unsupported method: (startPrompt|streamEventsSse)|(startPrompt|streamEventsSse) unsupported/i.test(li(i))}async function vp(i,p,b,h="",f=N(i).path,w=N(i).id){return await V(i,"submitPrompt",Ki(p,b,h,f),w)}function Ki(i,p,b,h){let f={text:i,attachments:p,sessionId:b};if(h)f.workspacePath=h;return f}function kp(i){let p=[...i.messages].reverse().find((h)=>h.role==="assistant"&&h.streaming);if(p)return p;let b={id:D(),role:"assistant",text:"",createdAt:Date.now(),streaming:!0};return i.messages.push(b),b}function up(i,p){for(let b of p)if(b.type==="text.delta"&&typeof b.delta==="string")i.text+=b.delta;else if(b.type==="thinking.delta"&&typeof b.delta==="string")i.thinking=`${i.thinking||""}${b.delta}`;else if(b.type==="tool.start")s(i,b,"running");else if(b.type==="tool.delta"&&typeof b.delta==="string"){let h=s(i,b,"running");h.text=b.delta}else if(b.type==="tool.end"){let h=s(i,b,b.isError?"err":"ok");if(typeof b.result==="string"&&b.result)h.text=b.result}else if(b.type==="error"&&typeof b.message==="string")i.text+=`${i.text?`
`:""}${b.message}`}function s(i,p,b){let h=p.toolCallId||p.toolName||"tool";i.toolCalls||=[];let f=i.toolCalls.find((w)=>w.id===h);if(!f)f={id:h,name:p.toolName||"tool",args:p.args,text:"",status:b},i.toolCalls.push(f);if(f.status=b,xp(f,p))f.args=p.args,f.argsStatus=p.argsStatus;return f}function xp(i,p){if(!p.argsStatus)return Boolean(p.args);if(p.argsStatus==="unavailable")return!i.argsStatus;return!0}function Ip(i){return A(i)&&typeof i.type==="string"}async function n(i,p,b,h,f="",w=hi(i),r=N(i).id){if(!i.backendStream){await Fi(i,p,b,h,f,w);return}let c=++h.backendChatToken;h.sessionEventsAbort?.abort();let l=new AbortController;h.sessionEventsAbort=l;let L=Yi(()=>{R(p,W(b).messages,b.activeSessionId)});try{let T=await Xi(i,"sessionEventsSse",Wi(i,f,w),l.signal,r);if(!T)throw Error("session SSE backend did not return a stream");await Mi(T,(O)=>{if(c!==h.backendChatToken||O.type!=="chat.state")return;if(vi(i,b,dp(O),"chatState").length)L.request()})}catch(T){if(!l.signal.aborted)await Fi(i,p,b,h,f,w)}finally{if(L.cancel(),h.sessionEventsAbort===l)h.sessionEventsAbort=void 0}}async function Fi(i,p,b,h,f="",w=hi(i)){let r=++h.backendChatToken;try{let c=await V(i,"chatState",Wi(i,f,w));if(r!==h.backendChatToken)return;let l=vi(i,b,c,"chatState");if(l.length)R(p,l,b.activeSessionId)}catch{}}function Wi(i,p,b=hi(i)){let h=p?{sessionId:p}:{};if(b)h.workspacePath=b;return h}function dp(i){return{activeSessionId:i.activeSessionId,messages:i.messages,isStreaming:i.isStreaming}}function hi(i){return N(i).path}function N(i){let p=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),b=p?.activeWorkspaceId||i.app?.dataset.activeWorkspaceId||"",h=p?.workspaces?.find((f)=>f.id===b)?.path||"";return{id:b,path:h}}function Pi(i,p){let b=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),h=p.workspaceId||b?.activeWorkspaceId||i.app?.dataset.activeWorkspaceId||"",f=b?.workspaces?.find((w)=>w.id===h)?.path||"";if(f||h!==b?.activeWorkspaceId)return{id:h,path:f};return N(i)}function vi(i,p,b,h){let f=ri(b.messages);if(typeof b.activeSessionId==="string"&&b.activeSessionId){let c=p.activeSessionId;if(G(p,b.activeSessionId),Z(i,{sessionId:b.activeSessionId,workspaceId:i.app?.dataset.activeWorkspaceId||S(i)||void 0}),h!=="chatState"||c!==b.activeSessionId)Q(i,"chat-session",{reason:h,sessionId:b.activeSessionId});if(h!=="chatState")bi(i,b.activeSessionId,h)}let w=W(p);if(!f.length)return[];let r=ti(w.messages,f).slice(-M);if(!ki(w.messages,r))return[];if(w.messages=r,w.title==="New chat"){let c=w.messages.find((l)=>l.role==="user");if(c)w.title=c.text.slice(0,48)||w.title}return w.updatedAt=Date.now(),E(p),w.messages}function ki(i,p){if(i.length!==p.length)return!0;return i.some((b,h)=>Ui(b)!==Ui(p[h]))}function Ui(i){return JSON.stringify({id:i.id,role:i.role,text:i.text,thinking:i.thinking,streaming:i.streaming,toolCalls:i.toolCalls,attachments:i.attachments})}function ap(i,p,b,h,f){let w=ri(b.messages),r=typeof b.activeSessionId==="string"&&b.activeSessionId?b.activeSessionId:f,c=p.activeSessionId===f;if(r!==f)if(c)G(p,r),Z(i,{sessionId:r,workspaceId:N(i).id||void 0}),Q(i,"chat-session",{reason:h,sessionId:r}),bi(i,r,h);else U(p,r);let l=U(p,r);if(!w.length)return[];let L=ti(l.messages,w).slice(-M);if(!ki(l.messages,L))return[];return l.messages=L,l.updatedAt=Date.now(),E(p),l.messages}function Sp(i,p,b,h,f){let w=(L)=>{if(!L?.sessionId)return;if(xi(p,h,L))return;Z(p,L),G(h,L.sessionId),R(b,W(h).messages,h.activeSessionId);let T=Pi(p,L);n(p,b,h,f,L.sessionId,T.path,T.id)},r=(L)=>{if(np(p,b,h,f,L))return;let T=op(p,L);if(T)w(T)},c=p.app?.piWebSidebar?.channels?.selectedSession$||globalThis.piWebSidebar?.channels?.selectedSession$,l=p.app?.piWebSidebar?.channels?.events$||globalThis.piWebSidebar?.channels?.events$;if(c)i.add(c.subscribe(w));if(l)i.add(l.subscribe(r));if(p.app)i.listen(p.app,"pi-web-sidebar:session-created",(L)=>{let T=L.detail||{},O=typeof T.sessionId==="string"?T.sessionId:"",$=typeof T.workspaceId==="string"?T.workspaceId:"";if(O)w({sessionId:O,workspaceId:$||void 0})});if(globalThis.piWeb)i.add(globalThis.piWeb.behaviorSubject(Di,K(p)).subscribe(w)),i.add(globalThis.piWeb.subject(Gi).subscribe(r)),i.add(globalThis.piWeb.behaviorSubject("session.activeId",null).subscribe((L)=>{if(!L)return;w({sessionId:L,workspaceId:p.app?.dataset.activeWorkspaceId||d(a)||void 0})}))}function ui(i,p){let b=K(i);if(b?.sessionId&&!xi(i,p,b))Z(i,b),G(p,b.sessionId);return p.activeSessionId}function xi(i,p,b){let h=b.workspaceId||"",f=i.app?.dataset.activeWorkspaceId||d(a)||"";return p.activeSessionId===b.sessionId&&(!h||h===f)}function np(i,p,b,h,f){if(f.type!=="session.deleted")return!1;let w=Ii(f);if(!w)return!0;let r=b.activeSessionId===w,c=b.sessions.length;if(b.sessions=b.sessions.filter((l)=>l.id!==w),r){h.runEventsAbort?.abort(),h.sessionEventsAbort?.abort();let l=mp(i,f,w);if(l?.sessionId){Z(i,l),G(b,l.sessionId),sp(p,W(b).messages,b.activeSessionId);let L=Pi(i,l);return n(i,p,b,h,l.sessionId,L.path,L.id),!0}return b.activeSessionId="",qp(i),E(b),fi(p),!0}if(b.sessions.length!==c)E(b);return!0}function mp(i,p,b){let h=p.snapshot?.activeSessionId||"",f=p.snapshot?.activeWorkspaceId||"";if(h&&h!==b)return{sessionId:h,workspaceId:f||void 0};let w=K(i);if(w?.sessionId&&w.sessionId!==b)return w;return null}function op(i,p){if(!tp(p.type))return null;let b=p.detail||{},h=Ii(p)||p.snapshot?.activeSessionId||"",f=typeof b.workspaceId==="string"?b.workspaceId:p.snapshot?.activeWorkspaceId||i.app?.dataset.activeWorkspaceId||"";if(!h)return null;return{sessionId:h,workspaceId:f||void 0}}function Ii(i){let p=i.detail||{};if(typeof p.sessionId==="string")return p.sessionId;if(typeof p.id==="string")return p.id;return""}function tp(i){return i==="session.selected"||i==="session.created"||i==="new-session"||i==="active.start"}function G(i,p){let b=i.sessions.find((h)=>h.id===p);if(!b)b=I(p),i.sessions.unshift(b);return i.activeSessionId=b.id,E(i),b}function R(i,p,b){if(Si(p,b),!p.length){fi(i);return}(i.querySelector(".term-inner")||i).replaceChildren(...p.map((f)=>ai(f,b))),wi(i)}function sp(i,p,b){Si(p,b),(i.querySelector(".term-inner")||i).replaceChildren(...p.map((f)=>ai(f,b))),wi(i)}function fi(i){let p=i.querySelector(".term-inner")||i,b=document.createElement("article");b.className="pi-web-chat-docs",b.setAttribute("aria-label","pi-web-chat guide");let h=document.createElement("h1");h.textContent="pi-web-chat guide";let f=document.createElement("p");f.textContent=["Select or create a session in the sidebar to load chat history,","or type below to start a new session."].join(" ");let w=document.createElement("ul");for(let r of["Ask pi in the prompt box and press Cmd/Ctrl+Enter to send.","Type ! then Space at the start to turn the prompt yellow and run shell commands in the workspace.","Queued file attachments hide during shell mode and reappear for the next normal prompt.","Type @ to list project files, then pick one to tag it as prompt context.","Type / at the start to open the slash command list.","Chats are cached locally after you start or select a session."]){let c=document.createElement("li");c.textContent=r,w.append(c)}b.append(h,f,w),p.replaceChildren(b),wi(i)}function ep(i,p){let b=p.querySelector(".term"),h=p.querySelector("[data-action='scroll-bottom']");if(!b||!h)return;let f={term:b,button:h,pinned:!0,touchStartY:null};ii.set(p,f),h.hidden=!1,x(f),i.listen(h,"click",()=>{f.pinned=!0,di(f),x(f)}),i.listen(b,"wheel",(w)=>{if(w.deltaY<0)Zi(f)}),i.listen(b,"touchstart",(w)=>{let r=w;f.touchStartY=r.touches.item(0)?.clientY??null}),i.listen(b,"touchmove",(w)=>{let c=w.touches.item(0)?.clientY;if(typeof c==="number"&&f.touchStartY!==null&&c-f.touchStartY>8)Zi(f)}),i.listen(b,"touchend",()=>{f.touchStartY=null}),i.add({remove:()=>{ii.delete(p)}})}function wi(i){let p=ii.get(i);if(!p)return;if(p.pinned)di(p);x(p)}function di(i){i.term.scrollTop=i.term.scrollHeight}function Zi(i){i.pinned=!1,x(i)}function x(i){i.button.dataset.pinned=i.pinned?"true":"false",i.button.setAttribute("aria-pressed",i.pinned?"true":"false")}function ai(i,p){let b=document.createElement("article");b.className="transcript-item",b.dataset.messageId=i.id;let h=document.createElement("div");h.className="msg",h.dataset.kind=e(i.role);let f=document.createElement("span");f.className=`prefix ${e(i.role)}`,f.textContent=_b(i.role);let w=document.createElement("pre");if(w.className=`body ${e(i.role)}`,w.textContent=i.text,h.append(f,w),ib(i))b.append(h);if(i.thinking)b.append(pb(i.thinking,Boolean(i.streaming)));for(let r of i.toolCalls||[])b.append(bb(r,ni(p,i,r)));if(i.streaming)b.dataset.streaming="true";return b}function ib(i){let p=i.text.trim().length>0;return i.role!=="assistant"||p}function pb(i,p){let b=document.createElement("details");b.className="msg-detail think thinking-block",b.open=p;let h=document.createElement("summary");h.className="label",h.textContent="THINKING";let f=document.createElement("pre");return f.className="body",f.textContent=i,b.append(h,f),b}function Si(i,p){let b=new Set;for(let h of i)for(let f of h.toolCalls||[])b.add(ni(p,h,f));for(let h of j)if(!b.has(h))j.delete(h)}function ni(i,p,b){return`${i}:${p.id}:${b.id}`}function bb(i,p){let b=document.createElement("div");b.className="tool-card",b.dataset.tool=i.name||"tool",b.dataset.status=i.status;let h=!j.has(p);b.dataset.collapsed=h?"true":"false";let f=document.createElement("button");if(f.type="button",f.className="tc-head",f.title=h?"Show tool output":"Hide tool output",f.setAttribute("aria-expanded",h?"false":"true"),f.setAttribute("aria-label",`${h?"Show":"Hide"} ${i.name||"tool"} output`),f.append(rb(i),lb(i),cb(i),Lb(i,h)),!h)b.append(mi(i));return f.addEventListener("click",()=>wb(b,f,i,p)),b.prepend(f),b}function mi(i){let p=document.createElement("pre");return p.className="tc-body",p.textContent=i.text||Tb(i),p}function hb(i,p){let b=0,h,f=p.ownerDocument.defaultView,w=typeof f?.matchMedia==="function"?f.matchMedia("(prefers-reduced-motion: reduce)"):void 0,r=()=>{if(h)clearInterval(h),h=void 0},c=()=>{b=(b+1)%Ji;for(let T of p.querySelectorAll(".spinner"))T.dataset.frame=String(b)},l=()=>{let T=p.querySelectorAll(".spinner");if(w?.matches||T.length===0){r(),b=0;for(let O of T)O.dataset.frame="0";return}if(!h)h=setInterval(c,Cp)},L=new(f?.MutationObserver||MutationObserver)(l);L.observe(p,{childList:!0,subtree:!0}),w?.addEventListener("change",l),l(),i.add({remove:()=>{L.disconnect(),w?.removeEventListener("change",l),r()}})}function fb(){let i=document.createElement("span");i.className="spinner",i.dataset.frame="0",i.setAttribute("aria-hidden","true");for(let p=0;p<Ji;p+=1)i.append(document.createElement("span"));return i}function wb(i,p,b,h){let f=i.querySelector(".tc-body"),w=f!==null;if(f)f.remove(),j.delete(h);else i.append(mi(b)),j.add(h);i.dataset.collapsed=w?"true":"false",p.setAttribute("aria-expanded",w?"false":"true"),p.setAttribute("aria-label",`${w?"Show":"Hide"} ${i.dataset.tool||"tool"} output`),p.title=w?"Show tool output":"Hide tool output";let r=p.querySelector(".tc-toggle-label");if(r)r.textContent=w?"show":"hide"}function rb(i){let p=document.createElement("span");p.className="tc-glyph";let b=Ob(i),h=b?_p[b]:"";if(!b||!h)return p.textContent="●",p;return p.innerHTML=`<svg class="tc-icon" data-tool-icon="${b}" aria-hidden="true" viewBox="0 0 24 24">${h}</svg>`,p}function lb(i){let p=document.createElement("span");return p.className="tc-name",p.textContent=i.name||"tool",p}function cb(i){let p=document.createElement("span");return p.className="tc-args",p.textContent=oi(i),p}function Lb(i,p){let b=document.createElement("span");if(b.className="tc-meta",i.status==="running"){let w=fb(),r=document.createElement("span");return r.className="running",r.textContent="running",b.append(w,r,Vi(p)),b}let h=document.createElement("span");h.className=i.status==="err"?"err":"ok",h.textContent=i.status==="err"?"✗":"✓";let f=document.createElement("span");return f.textContent=i.status==="err"?" · failed":" · done",b.append(h,f,Vi(p)),b}function Vi(i){let p=document.createElement("span");p.className="tc-toggle";let b=document.createElement("span");b.className="tc-toggle-label",b.textContent=i?"show":"hide";let h=document.createElement("span");return h.className="tc-caret",h.textContent="▸",p.append(b,h),p}function oi(i){if(i.argsStatus==="truncated")return"arguments truncated";if(i.argsStatus==="omitted")return"arguments omitted";if(i.argsStatus==="unavailable")return"arguments unavailable";if(i.argsStatus==="empty")return"no arguments";if(!i.args)return"";return JSON.stringify(i.args)}function Tb(i){if(i.argsStatus==="truncated")return"arguments truncated: too large to display";if(i.argsStatus==="omitted")return"arguments omitted: response too large";if(i.argsStatus==="unavailable")return"arguments unavailable";if(i.argsStatus==="empty")return"no arguments";if(!i.args)return"arguments unavailable";return JSON.stringify(i.args,null,2)}function Ob(i){let p=gb(i.name);if(["bash","shell","sh","zsh","terminal"].includes(p))return Cb(oi(i).toLowerCase())||"terminal";return Ep[p]}function gb(i){return i.trim().replace(/^functions\./,"").replace(/^multi_tool_use\./,"")}function Cb(i){if(/\bgit\b|\bgh\b/.test(i))return"git-branch";if(/\b(go test|bun test|npm test|pnpm test|yarn test|vitest|pytest|cargo test)\b/.test(i))return"circle-check";if(/\b(bun|npm|pnpm|yarn)\s+(run\s+)?build\b|\b(go build|cargo build)\b/.test(i))return"hammer";if(/\b(bun|npm|pnpm|yarn)\s+(add|install|i)\b|\bpip\s+install\b|\bgo\s+get\b/.test(i))return"package";return}function _b(i){if(i==="assistant")return"pi >";if(i==="user")return"you >";if(i==="system")return"sys >";return"tool >"}function e(i){if(i==="assistant")return"pi";if(i==="system")return"sys";return i}function Eb(i){let p=/^\/([^\s/]*)$/.exec(i);return p?p[1].toLowerCase():null}function Hb(i){let p=/(?:^|\s)@([^\s@`]*)$/.exec(i);return p?p[1]:null}function Qb(i){return i.length>0}function Kb(i,p){return i.trim().startsWith("!")&&p}function ti(i,p){let b=new Map;for(let h of i)b.set(h.id,h);for(let h of p)b.set(h.id,{...b.get(h.id),...h});return[...b.values()].sort((h,f)=>h.createdAt-f.createdAt)}function W(i){return U(i,i.activeSessionId,!0)}function U(i,p,b=!1){let h=i.sessions.find((f)=>f.id===p);if(!h)h=I(p||void 0),i.sessions.unshift(h),E(i);if(b||!i.activeSessionId)i.activeSessionId=h.id,E(i);return h}function $b(i,p,b,h){if(p===b)return;let f=i.sessions.find((l)=>l.id===p),w=f?.messages.findIndex((l)=>l.id===h)??-1;if(!f||w<0)return;let[r]=f.messages.splice(w,1),c=U(i,b);if(!c.messages.some((l)=>l.id===r.id))c.messages.push(r);f.updatedAt=Date.now(),c.updatedAt=Date.now(),E(i)}function I(i=D()){let p=Date.now();return{id:i,title:"New chat",createdAt:p,updatedAt:p,messages:[]}}function d(i){try{return localStorage.getItem(i)||""}catch{return""}}function yb(i=""){try{let b=JSON.parse(localStorage.getItem(u)||"null");if(b&&typeof b.activeSessionId==="string"&&Array.isArray(b.sessions)){let h=b.sessions.filter(ei).map(zb);if(i&&!h.some((f)=>f.id===i))h.unshift(I(i));return{activeSessionId:i||b.activeSessionId,sessions:h}}}catch{}let p=I(i||void 0);return{activeSessionId:p.id,sessions:[p]}}function Rb(){try{let i=JSON.parse(localStorage.getItem(u)||"null");return Boolean(i&&Array.isArray(i.sessions)&&i.sessions.some(ei))}catch{return!1}}function E(i){ji(i);try{localStorage.setItem(u,JSON.stringify(i))}catch{for(let p of i.sessions)p.messages=p.messages.slice(-Math.floor(M/2));ji(i);try{localStorage.setItem(u,JSON.stringify(i))}catch{}}}function si(i){if(!i?.length)return;return i.map(({content:p,...b})=>b)}function ji(i){i.sessions.sort((h,f)=>f.updatedAt-h.updatedAt);let p=i.sessions.find((h)=>h.id===i.activeSessionId),b=i.sessions.filter((h)=>h.id!==i.activeSessionId).slice(0,Math.max(0,yi-1));i.sessions=p?[p,...b]:i.sessions.slice(0,yi);for(let h of i.sessions)if(h.messages.length>M)h.messages.splice(0,h.messages.length-M)}function ei(i){return A(i)&&typeof i.id==="string"&&Array.isArray(i.messages)}function zb(i){return{...i,messages:ri(i.messages)}}function ri(i){return Array.isArray(i)?i.filter(Zb).map(Nb):[]}function Nb(i){if(!Array.isArray(i.toolCalls)){let{toolCalls:p,...b}=i;return b}return{...i,toolCalls:i.toolCalls.filter(Ab)}}function Ab(i){if(!A(i)||typeof i.id!=="string"||typeof i.name!=="string")return!1;return typeof i.text==="string"&&Ub(i.status)&&Fb(i.argsStatus)}function Fb(i){return i===void 0||i==="present"||i==="empty"||i==="unavailable"||i==="truncated"||i==="omitted"}function Ub(i){return i==="running"||i==="ok"||i==="err"}function Zb(i){if(!A(i)||typeof i.id!=="string"||typeof i.text!=="string")return!1;return typeof i.createdAt==="number"&&Vb(i.role)}function Vb(i){return i==="user"||i==="assistant"||i==="tool"||i==="system"}function A(i){return typeof i==="object"&&i!==null}function D(){return`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`}function li(i){return i instanceof Error?i.message:String(i)}export{Mb as submittedAttachmentsForText,Kb as shellAttachmentNoteVisible,pp as renderMessages,wp as promptFromAgUiLikeRunInput,Ei as pluginStyleText,o as pluginClass,Xb as mergeCommands,Qb as hasQueuedAttachmentNames,S as getActiveWorkspaceId,Fp as formatShellOutput,Bi as extractRefs,Hp as default,Ci as createComposerSurface,gi as createChatSurface,ip as createChatDom,Yb as createChannels,fp as createAgUiLikeRunInput,k as commandName,rp as chatEventsToAgUiLikeEvents,V as backendCall};
