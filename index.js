...
`);if(!r)return;try{let b=JSON.parse(r);if(di(b))p(b)}catch{}}function Ki(i,p){globalThis.piWeb?.subject("chat.input.submitted").next({text:i,attachments:Ei(p)||[]})}async function Xi(i,p,r,b){try{return await J(i,"startPrompt",{text:p,attachments:r,sessionId:b})}catch(h){if(Qi(h))return{};throw h}}function Qi(i){return/unknown method: (startPrompt|streamEventsSse)|unsupported method: (startPrompt|streamEventsSse)|(startPrompt|streamEventsSse) unsupported/i.test(yi(i))}async function Wi(i,p,r,b=""){return await J(i,"submitPrompt",{text:p,attachments:r,sessionId:b})}function vi(i){let p=_(i),r=[...p.messages].reverse().find((h)=>h.role==="assistant"&&h.streaming);if(r)return r;let b={id:F(),role:"assistant",text:"",createdAt:Date.now(),streaming:!0};return p.messages.push(b),b}function ni(i,p){for(let r of p)if(r.type==="text.delta"&&typeof r.delta==="string")i.text+=r.delta;else if(r.type==="thinking.delta"&&typeof r.delta==="string")i.thinking=`${i.thinking||""}${r.delta}`;else if(r.type==="tool.start")Z(i,r,"running");else if(r.type==="tool.delta"&&typeof r.delta==="string"){let b=Z(i,r,"running");b.text=r.delta}else if(r.type==="tool.end"){let b=Z(i,r,r.isError?"err":"ok");if(typeof r.result==="string"&&r.result)b.text=r.result}else if(r.type==="error"&&typeof r.message==="string")i.text+=`${i.text?`
`:""}${r.message}`}function Z(i,p,r){let b=p.toolCallId||p.toolName||"tool";i.toolCalls||=[];let h=i.toolCalls.find((c)=>c.id===b);if(!h)h={id:b,name:p.toolName||"tool",args:p.args,text:"",status:r},i.toolCalls.push(h);if(h.status=r,p.args)h.args=p.args;return h}function di(i){return R(i)&&typeof i.type==="string"}async function j(i,p,r,b,h=""){if(!i.backendStream){await ii(i,p,r,b,h);return}let c=++b.backendChatToken;b.sessionEventsAbort?.abort();let l=new AbortController;b.sessionEventsAbort=l;try{let f=await wi(i,"sessionEventsSse",Ti(i,h),l.signal);if(!f)throw Error("session SSE backend did not return a stream");await gi(f,(g)=>{if(c!==b.backendChatToken||g.type!=="chat.state")return;let w=Y(i,r,Si(g),"chatState");if(w.length)y(p,w)})}catch(f){if(!l.signal.aborted)await ii(i,p,r,b,h)}finally{if(b.sessionEventsAbort===l)b.sessionEventsAbort=void 0}}async function ii(i,p,r,b,h=""){let c=++b.backendChatToken;try{let l=await J(i,"chatState",Ti(i,h));if(c!==b.backendChatToken)return;let f=Y(i,r,l,"chatState");if(f.length)y(p,f)}catch{}}function Ti(i,p){let r=p?{sessionId:p}:{},b=Ii(i);if(b)r.workspacePath=b;return r}function Si(i){return{activeSessionId:i.activeSessionId,messages:i.messages,isStreaming:i.isStreaming}}function Ii(i){let p=i.app?.piWebSidebar?.getSnapshot?.()||globalThis.piWebSidebar?.getSnapshot?.(),r=i.app?.dataset.activeWorkspaceId||p?.activeWorkspaceId||"";return p?.workspaces?.find((b)=>b.id===r)?.path||""}function Y(i,p,r,b){let h=ai(r.messages);if(typeof r.activeSessionId==="string"&&r.activeSessionId){let l=p.activeSessionId;if(K(p,r.activeSessionId),z(i,{sessionId:r.activeSessionId,workspaceId:i.app?.dataset.activeWorkspaceId||x(i)||void 0}),b!=="chatState"||l!==r.activeSessionId)N(i,"chat-session",{reason:b,sessionId:r.activeSessionId});if(b!=="chatState")fi(i,r.activeSessionId,b)}let c=_(p);if(h.length){if(c.messages=Lp(c.messages,h).slice(-u),c.title==="New chat"){let l=c.messages.find((f)=>f.role==="user");if(l)c.title=l.text.slice(0,48)||c.title}c.updatedAt=Date.now()}return A(p),c.messages}function mi(i,p,r,b,h){let c=(w)=>{if(!w?.sessionId)return;if(w.sessionId!==b.activeSessionId)h.runEventsAbort?.abort();z(p,w),K(b,w.sessionId),y(r,_(b).messages),j(p,r,b,h,w.sessionId)},l=(w)=>{let L=oi(p,w);if(L)c(L)},f=p.app?.piWebSidebar?.channels?.selectedSession$||globalThis.piWebSidebar?.channels?.selectedSession$,g=p.app?.piWebSidebar?.channels?.events$||globalThis.piWebSidebar?.channels?.events$;if(f)i.add(f.subscribe(c));if(g)i.add(g.subscribe(l));if(p.app)i.listen(p.app,"pi-web-sidebar:session-created",(w)=>{let L=w.detail||{},T=typeof L.sessionId==="string"?L.sessionId:"",O=typeof L.workspaceId==="string"?L.workspaceId:"";if(T)c({sessionId:T,workspaceId:O||void 0})});if(globalThis.piWeb)i.add(globalThis.piWeb.behaviorSubject(bi,q(p)).subscribe(c)),i.add(globalThis.piWeb.subject(hi).subscribe(l)),i.add(globalThis.piWeb.behaviorSubject("session.activeId",null).subscribe((w)=>{if(!w)return;c({sessionId:w,workspaceId:p.app?.dataset.activeWorkspaceId||V(G)||void 0})}))}function oi(i,p){if(!ti(p.type))return null;let r=p.detail||{},b=typeof r.sessionId==="string"?r.sessionId:p.snapshot?.activeSessionId||"",h=typeof r.workspaceId==="string"?r.workspaceId:p.snapshot?.activeWorkspaceId||i.app?.dataset.activeWorkspaceId||"";if(!b)return null;return{sessionId:b,workspaceId:h||void 0}}function ti(i){return i==="session.selected"||i==="session.created"||i==="new-session"||i==="active.start"}function K(i,p){let r=i.sessions.find((b)=>b.id===p);if(!r)r=k(p),i.sessions.unshift(r);return i.activeSessionId=r.id,A(i),r}function y(i,p){(i.querySelector(".term-inner")||i).replaceChildren(...p.map(ei))}function ei(i){let p=document.createElement("article");p.className="transcript-item",p.dataset.messageId=i.id;let r=document.createElement("div");r.className="msg",r.dataset.kind=B(i.role);let b=document.createElement("span");b.className=`prefix ${B(i.role)}`,b.textContent=gp(i.role);let h=document.createElement("pre");if(h.className=`body ${B(i.role)}`,h.textContent=i.text,r.append(b,h),p.append(r),i.thinking)p.append(si(i.thinking,Boolean(i.streaming)));for(let c of i.toolCalls||[])p.append(ip(c));if(i.streaming)p.dataset.streaming="true";return p}function si(i,p){let r=document.createElement("details");r.className="msg-detail think thinking-block",r.open=p;let b=document.createElement("summary");b.className="label",b.textContent="THINKING";let h=document.createElement("pre");return h.className="body",h.textContent=i,r.append(b,h),r}function ip(i){let p=document.createElement("div");p.className="tool-card",p.dataset.tool=i.name||"tool",p.dataset.status=i.status;let r=i.status!=="running";p.dataset.collapsed=r?"true":"false";let b=document.createElement("button");if(b.type="button",b.className="tc-head",b.title=r?"Show tool output":"Hide tool output",b.setAttribute("aria-expanded",r?"false":"true"),b.setAttribute("aria-label",`${r?"Show":"Hide"} ${i.name||"tool"} output`),b.append(rp(i),bp(i),hp(i),cp(i)),!r)p.append(Ci(i));return b.addEventListener("click",()=>pp(p,b,i)),p.prepend(b),p}function Ci(i){let p=document.createElement("pre");return p.className="tc-body",p.textContent=i.text||JSON.stringify(i.args||{},null,2),p}function pp(i,p,r){let b=i.querySelector(".tc-body"),h=b!==null;if(b)b.remove();else i.append(Ci(r));i.dataset.collapsed=h?"true":"false",p.setAttribute("aria-expanded",h?"false":"true"),p.setAttribute("aria-label",`${h?"Show":"Hide"} ${i.dataset.tool||"tool"} output`),p.title=h?"Show tool output":"Hide tool output";let c=p.querySelector(".tc-toggle-label");if(c)c.textContent=h?"show":"hide"}function rp(i){let p=document.createElement("span");p.className="tc-glyph";let r=lp(i),b=r?zi[r]:"";if(!r||!b)return p.textContent="●",p;return p.innerHTML=`<svg class="tc-icon" data-tool-icon="${r}" aria-hidden="true" viewBox="0 0 24 24">${b}</svg>`,p}function bp(i){let p=document.createElement("span");return p.className="tc-name",p.textContent=i.name||"tool",p}function hp(i){let p=document.createElement("span");return p.className="tc-args",p.textContent=Oi(i),p}function cp(i){let p=document.createElement("span");if(p.className="tc-meta",i.status==="running"){let h=document.createElement("span");h.className="spinner",h.textContent="⠇";let c=document.createElement("span");return c.className="running",c.textContent="running",p.append(h,c,pi(i.status)),p}let r=document.createElement("span");r.className=i.status==="err"?"err":"ok",r.textContent=i.status==="err"?"✗":"✓";let b=document.createElement("span");return b.textContent=i.status==="err"?" · failed":" · done",p.append(r,b,pi()),p}function pi(i="ok"){let p=document.createElement("span");p.className="tc-toggle";let r=document.createElement("span");r.className="tc-toggle-label",r.textContent=i==="running"?"hide":"show";let b=document.createElement("span");return b.className="tc-caret",b.textContent="▸",p.append(r,b),p}function Oi(i){if(!i.args)return"";return JSON.stringify(i.args)}function lp(i){let p=fp(i.name);if(["bash","shell","sh","zsh","terminal"].includes(p))return wp(Oi(i).toLowerCase())||"terminal";return Fi[p]}function fp(i){return i.trim().replace(/^functions\./,"").replace(/^multi_tool_use\./,"")}function wp(i){if(/\bgit\b|\bgh\b/.test(i))return"git-branch";if(/\b(go test|bun test|npm test|pnpm test|yarn test|vitest|pytest|cargo test)\b/.test(i))return"circle-check";if(/\b(bun|npm|pnpm|yarn)\s+(run\s+)?build\b|\b(go build|cargo build)\b/.test(i))return"hammer";if(/\b(bun|npm|pnpm|yarn)\s+(add|install|i)\b|\bpip\s+install\b|\bgo\s+get\b/.test(i))return"package";return}function gp(i){if(i==="assistant")return"pi >";if(i==="user")return"you >";if(i==="system")return"sys >";return"tool >"}function B(i){if(i==="assistant")return"pi";if(i==="system")return"sys";return i}function Lp(i,p){let r=new Map;for(let b of i)r.set(b.id,b);for(let b of p)r.set(b.id,{...r.get(b.id),...b});return[...r.values()].sort((b,h)=>b.createdAt-h.createdAt)}function _(i){let p=i.sessions.find((r)=>r.id===i.activeSessionId);if(!p)p=k(),i.sessions.unshift(p),i.activeSessionId=p.id,A(i);return p}function k(i=F()){let p=Date.now();return{id:i,title:"New chat",createdAt:p,updatedAt:p,messages:[]}}function V(i){try{return localStorage.getItem(i)||""}catch{return""}}function Tp(i=""){try{let r=JSON.parse(localStorage.getItem(D)||"null");if(r&&typeof r.activeSessionId==="string"&&Array.isArray(r.sessions)){let b=r.sessions.filter(Cp).map(Op);if(i&&!b.some((h)=>h.id===i))b.unshift(k(i));return{activeSessionId:i||r.activeSessionId,sessions:b}}}catch{}let p=k(i||void 0);return{activeSessionId:p.id,sessions:[p]}}function A(i){ri(i);try{localStorage.setItem(D,JSON.stringify(i))}catch{for(let p of i.sessions)p.messages=p.messages.slice(-Math.floor(u/2));ri(i);try{localStorage.setItem(D,JSON.stringify(i))}catch{}}}function Ei(i){if(!i?.length)return;return i.map(({content:p,...r})=>r)}function ri(i){i.sessions.sort((b,h)=>h.updatedAt-b.updatedAt);let p=i.sessions.find((b)=>b.id===i.activeSessionId),r=i.sessions.filter((b)=>b.id!==i.activeSessionId).slice(0,Math.max(0,t-1));i.sessions=p?[p,...r]:i.sessions.slice(0,t);for(let b of i.sessions)if(b.messages.length>u)b.messages.splice(0,b.messages.length-u)}function Cp(i){return R(i)&&typeof i.id==="string"&&Array.isArray(i.messages)}function Op(i){return{...i,messages:ai(i.messages)}}function ai(i){return Array.isArray(i)?i.filter(_p).map(Ep):[]}function Ep(i){if(!Array.isArray(i.toolCalls)){let{toolCalls:p,...r}=i;return r}return{...i,toolCalls:i.toolCalls.filter(ap)}}function ap(i){if(!R(i)||typeof i.id!=="string"||typeof i.name!=="string")return!1;return typeof i.text==="string"&&yp(i.status)}function yp(i){return i==="running"||i==="ok"||i==="err"}function _p(i){if(!R(i)||typeof i.id!=="string"||typeof i.text!=="string")return!1;return typeof i.createdAt==="number"&&Ap(i.role)}function Ap(i){return i==="user"||i==="assistant"||i==="tool"||i==="system"}function R(i){return typeof i==="object"&&i!==null}function F(){return`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`}function yi(i){return i instanceof Error?i.message:String(i)}export{Ri as renderMessages,$i as promptFromAgUiLikeRunInput,S as pluginStyleText,U as pluginClass,zp as mergeCommands,x as getActiveWorkspaceId,kp as extractRefs,Pi as default,v as createComposerSurface,W as createChatSurface,Ai as createChatDom,Np as createChannels,Mi as createAgUiLikeRunInput,m as commandName,xi as chatEventsToAgUiLikeEvents,J as backendCall};
