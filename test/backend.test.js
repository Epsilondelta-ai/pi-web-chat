...
async function waitForFileIncludes(path, expected) {
  for (let index = 0; index < 30; index += 1) {
    try {
      ...
      // File is created by the fake pi process.
    }

    await new Promise((resolve) => setTimeout(resolve, 50));
  }

  assert.fail(`${path} did not include ${expected}`);
}
...
test("steerPrompt sends queued input to the active streaming pi rpc process", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-chat-workspace-"));
  const home = await mkdtemp(join(tmpdir(), "pi-web-chat-home-"));
  const bin = await mkdtemp(join(tmpdir(), "pi-web-chat-bin-"));
  const marker = join(home, "pi-steer.jsonl");
  const fakePi = join(bin, "pi");
  await writeFile(fakePi, `#!/usr/bin/env node
const fs = require('fs');
let buffer = '';
...
process.stdin.on('data', chunk => {
  ...
  while (buffer.includes('\\n')) {
    ...
    if (payload.type === 'steer') {
      fs.appendFileSync(process.env.PI_STEER_MARKER, JSON.stringify(payload) + '\\n');
...
`);
  ...
  const steer = await callBackend("steerPrompt", root, { runId: start.runId, text: "second" }, env);
